...
export default function RecordsScreen({ token }: { token: string }) {
  ...
  return (
    <View style={styles.container}>
      ...
      {mode === 'history' ? <AttendanceHistory token={token} /> : <Rankings token={token} />}
    </View>
  );
}
...
function AttendanceHistory({ token }: { token: string }) {
  ...
  const loadWeeks = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_URL}/api/attendance/weeks`, {
...
function WeekDetailView({ token, week, onBack }: { token: string; week: Week; onBack: () => void }) {
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    (async () => {
      ...
      setError('');
      try {
        const res = await fetch(`${API_URL}/api/attendance/week/${week.week_start}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
  ...
  const toggleDay = async (date: string, isHoliday: boolean) => {
    ...
    setLoadingDay(date);
    try {
      const res = await fetch(`${API_URL}/api/attendance/day/${date}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok) {
...
function Rankings({ token }: { token: string }) {
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    (async () => {
      ...
      setError('');
      try {
        const res = await fetch(`${API_URL}/api/assessment/rankings`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
