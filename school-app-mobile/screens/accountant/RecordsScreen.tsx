// screens/accountant/RecordsScreen.tsx
// Records & Reports — all-time gross total, plus school-wide and per-class totals
// for a selectable range (day/week/month).
...
export default function RecordsScreen({ token, onBack }: { token: string; onBack: () => void }) {
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError('');
    try {
      const [grossRes, schoolRes] = await Promise.all([
        fetch(`${API_URL}/api/accountant/records/gross`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${API_URL}/api/accountant/records/school?range=${range}`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
      const grossData = await grossRes.json();
      if (!grossRes.ok) throw new Error(grossData.error || 'Failed to load gross total');
      const schoolData = await schoolRes.json();
      if (!schoolRes.ok) throw new Error(schoolData.error || 'Failed to load records');

      setGrossTotal(grossData.gross_total);
      setRangeTotal(schoolData.total);
      setRangeStart(schoolData.start);
      setRangeEnd(schoolData.end);
      setByClass(schoolData.by_class);
    } catch (e: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Records & Reports</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  grossCard: {
    backgroundColor: colors.indigo, borderRadius: radii.lg, padding: spacing.lg,
  ...
  rangeButton: {
    flex: 1, borderWidth: 1, borderColor: colors.line, borderRadius: radii.pill,
