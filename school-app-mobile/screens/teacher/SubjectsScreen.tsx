...
export default function SubjectsScreen({
  ...
  const loadSubjects = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_URL}/api/assessment/subjects?classId=${classId}`, {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const res = await fetch(`${API_URL}/api/assessment/subjects/${subject.id}?classId=${classId}`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${token}` },
              });
              const data = await res.json();
  ...
  const addSubject = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/assessment/subjects`, {
        ...
        body: JSON.stringify({ name: newSubjectName.trim(), classId }),
      });
      const data = await res.json();
