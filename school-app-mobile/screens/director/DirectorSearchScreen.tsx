// screens/director/DirectorSearchScreen.tsx
...
export default function DirectorSearchScreen({
  token,
  onBack,
  onSelectStudent,
}: {
  ...
  const runSearch = async () => {
    ...
    setSearched(true);
    try {
      const res = await fetch(`${API_URL}/api/director/all-students/search?q=${encodeURIComponent(query.trim())}`, {
                ...
                <View style={styles.card}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.studentSubtext}>
                      {item.class_name} · {item.admission_number || 'No admission number'}
                    </Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  searchInput: {
    flex: 1, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: radii.md,
    padding: spacing.md, fontFamily: fonts.body, fontSize: fontSizes.base, color: colors.charcoal,
  },
  searchButton: { backgroundColor: colors.indigo, borderRadius: radii.md, paddingHorizontal: spacing.lg, alignItems: 'center', justifyContent: 'center' },
